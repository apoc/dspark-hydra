...
> - **Draft size ≈ 265M active (dense) / 454M total, ~278M active (MoE, K=16, k'=2)** at the real config (V=248320, hidden 2048, dense FFN 1024, 4 layers). Any "~2M" is a toy-vocab (V=512) test artifact. This is a **deliberately small budget** to isolate router reuse at *equal active params* — **~an order of magnitude below production DSpark drafts**: RedHat's public [GLM-5.2 DSpark speculator](https://huggingface.co/RedHatAI/GLM-5.2-speculator.dspark) is **3.807B params total**, a 5-layer backbone at **hidden 6144 / intermediate 12288** (block 8, 7 speculative tokens). Its card reports accepted length **3.967** at acceptance rate 0.584; since 3.967/7≈0.57 matches, that count **excludes the bonus token** (unlike our τ=n_acc+1) — i.e. ≈4.97 in our convention, so raw figures are not directly comparable. Its backbone alone (~1.6B) is ~6× our whole active budget, total ~14× (trainable/frozen split not published). So **draft capacity is a genuine, untested acceptance lever alongside training-data scale**; this experiment tests router reuse only, NOT whether DSpark can reach production τ here.
