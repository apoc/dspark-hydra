...
> - **Draft size ≈ 265M active (dense) / 454M total, ~278M active (MoE, K=16, k'=2)** at the real config (V=248320, hidden 2048, dense FFN 1024, 4 layers). Any "~2M" is a toy-vocab (V=512) test artifact. This is a **deliberately small budget** to isolate router reuse at *equal active params* — **~an order of magnitude below production DSpark drafts**: RedHat's public [GLM-5.2 DSpark speculator](https://huggingface.co/RedHatAI/GLM-5.2-speculator.dspark) is **3.807B params total**, a 5-layer backbone at **hidden 6144 / intermediate 12288** (block 8, 7 draft slots). Its accepted length **3.967 is DSpark τ in the same bonus-inclusive `n_acc+1` convention** as ours — Speculators [`metrics.py`](https://github.com/vllm-project/speculators/blob/main/src/speculators/models/dspark/metrics.py): `accept_prefix.sum()+1`, "DSpark's tau", max 8 — so **normalized ≈0.50 vs our B3 0.29** (1.751/6). (Theirs is the analytical overlap-based expected τ; ours empirical from sampling. Their reported 0.584 is mean per-position overlap `Σ min(q,p)`, a different quantity.) Its backbone alone (~1.6B) is ~6× our whole active budget, total ~14×. So **draft capacity is a genuine, untested acceptance lever alongside training-data scale**; this experiment tests router reuse only, NOT whether DSpark can reach production τ here.
